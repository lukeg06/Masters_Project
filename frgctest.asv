...
for i  = 1
   set try
        ...
        try
            ...
            saveFileName = strcat(savePath,'ERROR_frgc_',num2str(i));
            errorS = 'ERROR';
            save(saveFileName,'errorS');
            continue;
        end
        ...
        %%
        estimate = pixel2mm([(size(imageIn2D,2)./2)+mm2pixel(prn(1)),(size(imageIn2D,1)./2)-mm2pixel(prn(2))]);
        PRNLocation = localisePRN(imageIn,estimate,8,'false');
